...
for r = 1:nReefs
    ...
    for p = 1:nParts
        ...
        if inpolygon(GBRShape(r).Centroid(1), GBRShape(r).Centroid(2), ...
                squeeze(partArr(p, 1, :)), squeeze(partArr(p, 2, :)))
            break
        end
    ...
    if maxX > partArr(p, 1, 3)
        ...
        if maxY > partArr(p, 2, 2)
            ...
            for i = 1:borderLength
                if inpolygon(GBRShape(r).X(i), GBRShape(r).Y(i), ...
                        squeeze(partArr(p+1-nCols, 1, :)), ...
                        squeeze(partArr(p+1-nCols, 2, :)))
        ...
        for i = 1:borderLength
            if inpolygon(GBRShape(r).X(i), GBRShape(r).Y(i), ...
                    squeeze(partArr(p+1, 1, :)), ...
                    squeeze(partArr(p+1, 2, :)))
    ...
    if maxY > partArr(p, 2, 2)
        ...
        for i = 1:borderLength
            if inpolygon(GBRShape(r).X(i), GBRShape(r).Y(i), ...
                    squeeze(partArr(p-nCols, 1, :)), ...
                    squeeze(partArr(p-nCols, 2, :)))
